...
from agents import (Agent, GuardrailFunctionOutput, RunContextWrapper, Runner,
                    TResponseInputItem, input_guardrail, output_guardrail)
    ...
    for pattern in python_patterns + shell_patterns:
        if pattern in input_text.lower():
            ...
            result = await Runner.run(
                code_injection_agent, input_text, context=ctx.context
            )
            ...
            return GuardrailFunctionOutput(
                output_info=output,
                tripwire_triggered=output.is_injection_attempt
                and output.risk_level in ["medium", "high"],
            )
...
async def check_shell_command_safety(
    ...
    if any(
        cmd in input_text.lower()
        for cmd in ["rm", "sudo", "chmod", "chown", "dd", ">", "|", "&"]
    ):
        ...
        return GuardrailFunctionOutput(
            output_info=output,
            tripwire_triggered=output.is_dangerous
            and output.risk_level in ["medium", "high"],
        )
...
async def moderate_content(
    ...
    return GuardrailFunctionOutput(
        output_info=output, tripwire_triggered=output.is_inappropriate
    )
...
async def ensure_safe_output(
    ...
    return GuardrailFunctionOutput(
        output_info="Output appears safe", tripwire_triggered=False
    )
