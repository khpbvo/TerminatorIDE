...
class ModelSelector:
    ...
    def get_recommended_settings(model_type: ModelType) -> ModelSettings:
        ...
        if model_type == ModelType.GPT4O:
            return ModelSettings(temperature=0.7, max_tokens=4000, top_p=1.0)
        elif model_type == ModelType.GPT4:
            return ModelSettings(temperature=0.7, max_tokens=4000, top_p=1.0)
        elif model_type == ModelType.GPT35TURBO:
            return ModelSettings(temperature=0.8, max_tokens=2000, top_p=1.0)
        elif model_type == ModelType.O3MINI:
            return ModelSettings(temperature=0.9, max_tokens=1000, top_p=1.0)
        else:
            # Default settings
