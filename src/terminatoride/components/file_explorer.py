...
from textual.widgets import Static, Tree, DirectoryTree, Label, Input, Button
...
from .modal import InputDialog, ConfirmDialog
...
class FileExplorer(Widget):
    ...
    def _populate_tree(self) -> None:
        ...
        try:
            ...
            if self.path != Path("/"):
                parent_node = tree.root.add(".. (Parent Directory)", data=str(self.path.parent))
                parent_node.expand()
        ...
        if event.node is not self.query_one(Tree).root:
            ...
            if path.is_dir():
                try:
                    ...
                    # Add directories
                    for directory in dirs:
                        node = event.node.add(f"📁 {directory.name}", data=str(directory))
                        # Add a placeholder child to show the expand button
                        if any(True for _ in directory.iterdir()):
                            node.add("⏳ Loading...")
            ...
            if matches:
                ...
                for path in sorted(matches, key=lambda p: str(p).lower()):
                    ...
                    for i, part in enumerate(parts):
                        ...
                        child = next((c for c in current.children if c.label == part), None)
                        ...
                        if child is None:
                            ...
                            if i == len(parts) - 1:  # Last part (file/dir name)
                                ...
                                child = current.add(part, data=str(self.path.joinpath(*parts[:i+1])))
                ...
                self.notify(f"Found {len(matches)} matches for '{query}'", severity="information")
            else:
    ...
    async def action_rename(self) -> None:
        ...
        new_name = await self.app.push_screen(
            InputDialog("Rename", f"Rename {self.current_selection.name} to:", self.current_selection.name)
        )
