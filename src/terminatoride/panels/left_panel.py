...
from textual.widgets import Header, Static, TabbedContent, TabPane, Rule, Button
...
class LeftPanel(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="tab-content"):
            ...
            yield FileExplorer(self.path, id="file-explorer", classes="tab-panel active")
            
            # Git Integration (initially hidden)
            yield Static("Git integration coming soon...", id="git-panel", classes="tab-panel")
            
            # SSH Remote Editing (initially hidden)
            yield Static("SSH remote editing coming soon...", id="ssh-panel", classes="tab-panel")
    ...
    def _set_active_tab(self, tab_id: str) -> None:
        ...
        self.query_one("#files-tab-btn").variant = "primary" if tab_id == "files" else "default"
        self.query_one("#git-tab-btn").variant = "primary" if tab_id == "git" else "default"
        self.query_one("#ssh-tab-btn").variant = "primary" if tab_id == "ssh" else "default"
