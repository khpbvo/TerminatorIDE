from terminatoride.config import AppConfig, Config, OpenAIConfig
...
class TestConfig:
    ...
    def test_save_config_to_file(self, tmp_path):
        ...
        # Create a config
        config = Config(
            openai=OpenAIConfig(api_key="test_key", model="gpt-4", temperature=0.8),
            app=AppConfig(
                theme="dark", editor_type="vim", show_line_numbers=True, auto_save=True
            ),
        )
